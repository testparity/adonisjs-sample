...
[![Parity Sample](https://github.com/testparity/adonisjs-sample/actions/workflows/parity.yml/badge.svg)](https://github.com/testparity/adonisjs-sample/actions/workflows/parity.yml)

This repository is a focused Parity demo for AdonisJS. It intentionally separates aggregate file coverage from matching-test coverage, which is the core Parity use case.
...
| --- | --- | ---: | ---: | ---: | --- |
| `app/services/user_presenter.ts` | `tests/unit/services/user_presenter.spec.ts` | 70% | 40% | 2 | Overall coverage looks acceptable, but most covered lines come from incidental tests. |
| `app/services/audit_summary.ts` | `tests/unit/services/audit_summary.spec.ts` | 90% | 90% | 2 | The matching test owns nearly all of the file's coverage. |
...
- `user_presenter.ts`: `minimum-coverage = 70%`, `matched-coverage = 40%`, `coverage-attribution = 3|2`.
- `audit_summary.ts`: `minimum-coverage = 90%`, `matched-coverage = 90%`, `coverage-attribution = 3|2`.
